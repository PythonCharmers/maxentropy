...
class FeatureTransformer(BaseEstimator, TransformerMixin):
    ...
    matrix_format : string
             Currently 'csr_array', 'csc_array', 'csr_matrix', 'csc_matrix', and
             'ndarray' are recognized.
    ...
    *,
    ...
    ----------
        ...
        matrix_format: str
            ...
            'csc_array', 'csr_array', 'csc_matrix', 'csr_matrix' etc. for
            constructing a scipy.sparse array / matrix of features.
...
def evaluate_feature_matrix(
    ...
    *,
    ...
    matrix_format : str (default 'csc_array')
        Options: 'ndarray', 'csc_array', 'csr_array', 'dok_array', 'csc_matrix',
        'csr_matrix'. If you have enough memory, it may be faster to create a
        dense ndarray and then construct a e.g. CSC array from this.

    Returns
    -------
    F : (n x d) array (in the given array format: ndarray / csc_array /
        csc_matrix etc.) Matrix or array of evaluated features.
    ...
    if matrix_format in ("dok_array", "csc_array", "csr_array"):
        F = scipy.sparse.dok_array((n, d), dtype=dtype)
    elif matrix_format in ("dok_matrix", "csc_matrix", "csr_matrix"):
    ...
    if matrix_format in {"csc_array", "csc_matrix"}:
        return F.tocsc()
    elif matrix_format in {"csr_array", "csr_matrix"}:
        return F.tocsr()
    else:
        return F
